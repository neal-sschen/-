...
for i in range(m):
    for j in range(k):
        tt.append(max(f2((x1+hm*i),vx,ax),f2(abs(y1+hk*j-y0),vy,ay)))

t0 = f2(d,vz1,az1)+f2(d,vz2,az2)+f2(h,vh,ah)+tt0
t_final = 2*sum(tt)/len(tt)+2*t0
nn = 3600/t_final*0.85
...
tt2 = [max(f2(1/5*LL,vx,ax),f2(2/3*HH,vy,ay)),max(f2(2/3*LL,vx,ax),f2(1/5*HH,vy,ay))]
t_final2 = 2*sum(tt2)/len(tt2)+2*t0
nn2 = 3600/t_final2*0.85
print('*****************')
print(nn2,t_final2,HH,LL)
